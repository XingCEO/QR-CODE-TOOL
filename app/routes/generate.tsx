...
export const meta: MetaFunction = () => {
  return [
    { title: '生成 QR Code - QR Code 工具' },
    { name: 'description', content: '每 2 秒自動生成包含當前時間戳的 QR Code' },
  ];
};
    ...
    <div className="flex flex-col min-h-screen items-center justify-center bg-gradient-to-br from-slate-900 to-slate-700 text-gray-100 p-6 font-sans">
      <div className="bg-slate-800 p-8 rounded-xl shadow-2xl w-full max-w-lg">
        <header className="mb-8 text-center">
          ...
          <p className="text-slate-400">每 2 秒自動生成包含當前時間戳的 QR Code</p>
        </header>

        <div className="flex flex-col items-center space-y-4">
          <button
            onClick={handleManualGenerate}
            className="bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-white font-semibold py-3 px-8 rounded-lg shadow-md transition-all duration-200"
          >
            手動生成新的 QR Code
          </button>
        </div>
        ...
        {latestData.error && (
          <div className="mt-4 p-4 bg-red-700 bg-opacity-50 border border-red-500 text-red-300 rounded-lg text-center">
        ...
        {latestData.qrCodeDataUrl && latestData.sourceText && (
          <div className="mt-6 flex flex-col items-center space-y-4">
            ...
            <div className="p-4 bg-slate-700 rounded-lg shadow-inner w-full text-center">
              <p className="text-slate-400">生成時間（台灣時間）：</p>
